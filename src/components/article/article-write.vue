<template>
  <div class="article-write">
    <el-card :body-style="{ padding: '10px' }">
        ...
        >
          ...
          <el-form-item label="标题" prop="articleTitle">
            ...
            >
              <template slot="append"
                >{{ article.articleTitle.length }}/30</template
              >
            </el-input>
          </el-form-item>
          <el-form-item label="内容">
            <base-markdown
              :init-value="article.articleMdContent"
              @save="onSave($event)"
            ></base-markdown>
          </el-form-item>
          <!-- 标签 -->
          <el-form-item label="标签" prop="articleTags">
            <el-card shadow="hover" :body-style="{ padding: '5px' }">
              ...
              <el-input
                ...
                style="width:80px;"
              ></el-input>
              <span style="color:#909399;padding:3px;float:right;"
                >{{ article.articleTags.length }}/10</span
              >
            </el-card>
          </el-form-item>
          <!-- 分区选择 -->
          <el-form-item label="分区" prop="articlePart">
            <el-select v-model="article.articlePart" placeholder="选择分区">
              <el-option
                v-for="(value, key) in partMap"
                :key="key"
                :label="value"
                :value="key"
              >
          ...
          <el-form-item label style="text-align:center;">
            <el-button
              type="primary"
              :disabled="isCommit"
              @click="onArticlePublish"
              >提交文章</el-button
            >
            <el-button
              type="button"
              :disabled="isTemp"
              @click="onArticleTempSave"
              >暂存草稿</el-button
            >
          </el-form-item>
        </el-form>
      ...
      <!-- 暂存成功页面 -->
      <div style="text-align:center;" v-else-if="isTemp && isSuccess">
        <h1 style="line-height:300px;">{{ result }}</h1>
        <div style="margin-top:50px;">
